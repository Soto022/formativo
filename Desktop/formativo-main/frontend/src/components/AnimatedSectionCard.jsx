    ...
    >
      <div className={`p-6 md:p-8 ${baseTransition} ${inView ? 'delay-200 opacity-100' : 'opacity-0'}`}>
        ...
        <div className="max-w-4xl mx-auto mb-8">
          <p className="text-lg whitespace-pre-wrap text-gray-300 break-words">{section.description}</p>
        </div>
        ...
        {section.subsections && section.subsections.map((subsection, subIndex) => (
          <div key={subIndex} className="max-w-4xl mx-auto mb-8 bg-slate-900/50 rounded-lg p-4">
            ...
            <p className="text-base md:text-lg whitespace-pre-wrap text-gray-300 break-words">{subsection.description}</p>
          </div>
        ))}
      </div>
      ...
      {section.images && section.images.length > 0 && (
        <div className={`${baseTransition} ${inView ? 'delay-300 opacity-100' : 'opacity-0'} pb-6 md:pb-8 bg-slate-900/50 px-6 md:px-8 overflow-x-hidden`}>
