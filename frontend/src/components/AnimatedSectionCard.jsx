...
export default function AnimatedSectionCard({ section, openLightbox }) {
  ...
  return (
    ...
    >
      <div className={`w-full p-6 md:p-8 ${baseTransition} ${inView ? 'delay-200 opacity-100' : 'opacity-0'}`}>
        {/* Main Title */}
        ...
        <div className="max-w-4xl mx-auto mb-8">
          <p className="text-base md:text-lg whitespace-pre-wrap break-words text-gray-300">{section.description}</p>
        </div>
        ...
        {section.subsections && section.subsections.map((subsection, subIndex) => (
          <div key={subIndex} className="max-w-4xl mx-auto mb-8 bg-slate-900/50 rounded-lg p-4">
            ...
            <p className="text-base md:text-lg whitespace-pre-wrap break-words text-gray-300">{subsection.description}</p>
          </div>
        ))}
      </div>
      ...
      {section.images && section.images.length > 0 && (
        <div className={`${baseTransition} ${inView ? 'delay-300 opacity-100' : 'opacity-0'} pb-6 md:pb-8 bg-slate-900/50 px-6 md:px-8`}>
          <h3 className="text-xl md:text-2xl font-semibold text-center mb-6 text-emerald-400">Galería de Imágenes</h3>
          <Swiper
